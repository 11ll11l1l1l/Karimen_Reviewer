...
class PMExecutionWorkspace(QWidget):
    ...
    def export_pptx(self):
        if not self.task:return
        path,_=QFileDialog.getSaveFileName(self,"Export PM Execution PowerPoint",f"{self.task.equipment_id}_{self.task.pm_id}_PM_{self.task.id}.pptx","PowerPoint (*.pptx)")
        if not path:return
        if not path.lower().endswith(".pptx"):path+=".pptx"
        try:export_pm_execution_pptx(self.db,self.task.id,path);QMessageBox.information(self,"PM report",f"Editable PM deck created.\n{path}")
        except Exception as exc:QMessageBox.critical(self,"PM report",str(exc))

    def export_xlsx(self):
        if not self.task:return
        path,_=QFileDialog.getSaveFileName(self,"Export PM Execution Excel",f"{self.task.equipment_id}_{self.task.pm_id}_PM_{self.task.id}.xlsx","Excel Workbook (*.xlsx)")
        if not path:return
        if not path.lower().endswith(".xlsx"):path+=".xlsx"
        try:export_pm_execution_xlsx(self.db,self.task.id,path);QMessageBox.information(self,"PM report",f"PM workbook created.\n{path}")
        except Exception as exc:QMessageBox.critical(self,"PM report",str(exc))
