...
from pdf_reporting import export_incident_pdf
...
class IncidentWorkspace(QWidget):
    ...
    def open_similar(self):
        ...
        if not row:return
        ticket_no=row.get("ticket_no","") if isinstance(row,dict) else row.ticket_no
        if ticket_no:self.set_ticket(ticket_no)
