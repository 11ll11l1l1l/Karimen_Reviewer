...
class RecordMention(Base):
    ...
    equipment_id: Mapped[str] = mapped_column(String(100), default="", index=True)
    username: Mapped[str] = mapped_column(String(80), index=True)
    mentioned_by: Mapped[str] = mapped_column(String(80), default="")
    mentioned_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, index=True)
...
class Database:
    ...
    def my_work(self, username: str, limit: int = 200) -> list[dict[str, Any]]:
        ...
        for mention in self.list_user_mentions(username,True,limit):
            rows.append({
                "severity":"MEDIUM","kind":"MENTION","key":str(mention.id),
                "entity_type":mention.entity_type,"entity_key":mention.entity_key,
                "equipment_id":mention.equipment_id,
                "summary":f"@mention from {mention.mentioned_by} on {mention.entity_type}:{mention.entity_key}",
                "owner":username,"age_hours":0.0,
            })
    ...
    def add_record_comment(
        ...
        with self.session() as s:
            comment=RecordComment(
                entity_type=et,entity_key=ek,equipment_id=equipment_id.strip(),
                body=body,created_by=user,
            )
            ...
            if not watcher:s.add(RecordWatcher(entity_type=et,entity_key=ek,username=user))
            names=set(re.findall(r"(?<![A-Za-z0-9_.-])@([A-Za-z0-9_.-]{1,80})",body))
            if names:
                valid={x.username for x in s.scalars(select(User).where(User.username.in_(names),User.active.is_(True)))}
                for username in valid:
                    ...
                    s.add(RecordMention(
                        ...
                        username=username,mentioned_by=user,
                    ))
            s.add(AuditLog(
                user=user,action="COMMENT_ADD",entity_type=et,entity_key=ek,
                detail=json.dumps({"comment_id":comment.id,"mentions":sorted(names)},sort_keys=True),
                workstation=workstation,
            ))
            self._queue_integration_event(s,"collaboration.comment.added",et,ek,{
                ...
                "created_by":user,"mentioned_users":sorted(names),
            })
            return comment
    ...
    def list_user_mentions(self, username: str, unread_only: bool = True, limit: int = 200):
        with self.session() as s:
            stmt=select(RecordMention).where(RecordMention.username==username)
            if unread_only:stmt=stmt.where(RecordMention.read_at.is_(None))
            return list(s.scalars(stmt.order_by(RecordMention.mentioned_at.desc()).limit(max(1,min(int(limit),1000)))))

    def mark_mention_read(self, mention_id: int, username: str):
        with self.session() as s:
