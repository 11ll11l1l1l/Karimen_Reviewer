...
class CollaborationPanel(QWidget):
    def __init__(self,db,user,parent=None):
        ...
        self.editor.setPlaceholderText("Add context, decision, update, or @mention a username…")
        self.editor.setMaximumHeight(110);root.addWidget(self.editor)
        foot=QHBoxLayout()
        hint=QLabel("Use @username to place this record in that person's My Work queue. Watchers receive comment updates.")
    ...
    def refresh(self):
        ...
        self.post_button.setEnabled(enabled);self.watch.setEnabled(enabled);self.editor.setEnabled(enabled)
        if not enabled:
        ...
        names=[x.username for x in watchers]
        self.watchers.setText(f"{len(names)} watcher(s)" + (f": {', '.join(names[:6])}" if names else ""))
    ...
    def post(self):
        ...
        if not body:return
        try:
            self.db.add_record_comment(
                self.entity_type,self.entity_key,body,self.user["username"],
                self.equipment_id,"COLLABORATION"
            )
