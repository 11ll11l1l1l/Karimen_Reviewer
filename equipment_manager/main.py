...
class PMDefinitionDialog(QDialog):
    def __init__(self,row=None,parent=None,initial=None):
        ...
        if row:
            ...
            self.pm.setText(str(initial.get("pm_id","") or ""));self.name.setText(str(initial.get("name","") or ""));self.eq.setText(str(initial.get("equipment_id","") or ""))
            self.type.setCurrentText(str(initial.get("schedule_type","Interval") or "Interval"));self.freq.setValue(float(initial.get("frequency_value",0) or 0));self.unit.setCurrentText(str(initial.get("frequency_unit","days") or "days"));self.anchor.setCurrentText(str(initial.get("anchor_mode","Original Due") or "Original Due"))
            self.early.setValue(int(initial.get("early_window_days",0) or 0));self.grace.setValue(int(initial.get("grace_days",0) or 0));self.hours.setValue(float(initial.get("estimated_hours",0) or 0));self.people.setValue(int(initial.get("required_people",1) or 1));self.skill.setText(str(initial.get("required_skill","") or ""));self.parts.setText(str(initial.get("required_parts","") or ""));self.sop.setText(str(initial.get("sop_path","") or ""))
...
class PMPage(QWidget):
    def __init__(self,db,user):
        ...
        wd=QWidget(); vd=QVBoxLayout(wd); hd=QHBoxLayout(); add=QPushButton("Add Definition"); templ=QPushButton("Add from Template");edit=QPushButton("Edit Definition"); gen=QPushButton("Generate Next PM"); ready=QPushButton("Parts Readiness"); add.clicked.connect(self.add_def);templ.clicked.connect(self.add_def_from_template); edit.clicked.connect(self.edit_def); gen.clicked.connect(self.generate_next); ready.clicked.connect(self.parts_ready); canedit=db.has_permission(user,"pm.edit"); add.setEnabled(canedit);templ.setEnabled(canedit); edit.setEnabled(canedit); gen.setEnabled(canedit); hd.addWidget(add);hd.addWidget(templ);hd.addWidget(edit);hd.addWidget(gen);hd.addWidget(ready);hd.addStretch(1);vd.addLayout(hd);self.def_table=make_table(["PM ID","Name","Equipment","Type","Frequency","Unit","Anchor","Early","Grace","Hours","Parts","Ver"]);vd.addWidget(self.def_table);self.tabs.addTab(wd,"Definitions")
    ...
    def add_def_from_template(self):
        ...
        if not templates:QMessageBox.information(self,"PM Template","No active PM definition templates are configured.");return
        labels=[f"{x.name} ({x.template_id})" for x in templates];choice,ok=QInputDialog.getItem(self,"PM Template","Template",labels,0,False)
        if not ok:return
        template=templates[labels.index(choice)]
        try:initial=self.db.apply_entity_template(template.template_id)
...
class TicketPage(QWidget):
    ...
    def add_from_template(self):
        ...
        if not initial.get("ticket_no"):initial["ticket_no"]=self.db.next_record_number("TICKET")
    ...
    def add(self):
        d=TicketDialog(parent=self,db=self.db,initial={"ticket_no":self.db.next_record_number("TICKET")})
        if d.exec()==QDialog.DialogCode.Accepted:
            try:
                row=self.db.save_ticket(d.data(self.user["username"]),workstation=WORKSTATION)
...
class QualificationProtocolDialog(QDialog):
    def __init__(self,row=None,parent=None,initial=None):
        ...
        if row:
            ...
            self.protocol.setText(str(initial.get("protocol_id","") or ""));self.name.setText(str(initial.get("name","") or ""));self.eq.setText(str(initial.get("equipment_id","") or ""));self.eqtype.setText(str(initial.get("equipment_type","") or ""))
            checks=initial.get("checks",[])
            if isinstance(checks,list):self.checks.setPlainText("\n".join(str(x.get("label","") if isinstance(x,dict) else x) for x in checks))
            elif checks:self.checks.setPlainText(str(checks))
    def data(self):
...
class QualificationPage(QWidget):
    def __init__(self,db,user):
        ...
        wp=QWidget();vp=QVBoxLayout(wp);hp=QHBoxLayout();newp=QPushButton("New Protocol");templp=QPushButton("New from Template");revp=QPushButton("New Revision")
        newp.clicked.connect(self.new_protocol);templp.clicked.connect(self.new_protocol_from_template);revp.clicked.connect(self.revise_protocol)
        newp.setEnabled(db.has_permission(user,"qualification.edit"));templp.setEnabled(db.has_permission(user,"qualification.edit"));revp.setEnabled(db.has_permission(user,"qualification.edit"))
        hp.addWidget(newp);hp.addWidget(templp);hp.addWidget(revp);hp.addStretch(1);vp.addLayout(hp)
        ...
        start=QPushButton("Start Run");result=QPushButton("Enter Result");submit=QPushButton("Submit");verify=QPushButton("Verify");approve=QPushButton("Approve");reject=QPushButton("Reject");ppt=QPushButton("Qualification PPTX");xlsx=QPushButton("Qualification Excel")
        start.clicked.connect(self.start_run);result.clicked.connect(self.enter_result);submit.clicked.connect(self.submit_run);verify.clicked.connect(self.verify_run);approve.clicked.connect(self.approve_run);reject.clicked.connect(self.reject_run);ppt.clicked.connect(self.export_pptx);xlsx.clicked.connect(self.export_xlsx)
    ...
    def new_protocol_from_template(self):
        ...
        if not templates:QMessageBox.information(self,"Qualification Template","No active qualification templates are configured.");return
        labels=[f"{x.name} ({x.template_id})" for x in templates];choice,ok=QInputDialog.getItem(self,"Qualification Template","Template",labels,0,False)
        if not ok:return
        template=templates[labels.index(choice)]
        try:initial=self.db.apply_entity_template(template.template_id)
