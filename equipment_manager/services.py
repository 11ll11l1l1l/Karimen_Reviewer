...
def read_clipboard_table(text: str) -> pd.DataFrame:
    text = (text or "").strip("\r\n")
    if not text: return pd.DataFrame()
    # Excel/LibreOffice clipboard is normally tab-separated with one row per line.
    return pd.read_csv(io.StringIO(text), sep="\t", dtype=object)
...
def normalize_pm_status(v: str) -> str:
    s = _norm(v)
    return {
        "open": "Pending", "pending": "Pending", "plan": "Scheduled", "planned": "Scheduled", "scheduled": "Scheduled",
        "wip": "In Progress", "in_progress": "In Progress", "delay": "Overdue", "late": "Overdue", "overdue": "Overdue",
...
def dataframe_to_pm_backlog(df: pd.DataFrame, mapping: dict[str, str]):
    ...
    for idx, r in df.iterrows():
        ...
        rows.append({
            "equipment_id": eq, "pm_id": pm, "pm_name": name or pm,
            "original_due_date": _dt(r.get(mapping.get("original_due_date", ""))), "scheduled_date": _dt(r.get(mapping.get("scheduled_date", ""))),
            "last_completion_date": _dt(r.get(mapping.get("last_completion_date", ""))), "status": normalize_pm_status(_text(r.get(mapping.get("status", "Pending")))),
            "assigned_to": _text(r.get(mapping.get("assigned_to", ""))), "estimated_hours": _num(r.get(mapping.get("estimated_hours", ""))) or 0.0,
            "priority": _text(r.get(mapping.get("priority", "Normal"))) or "Normal", "deferral_reason": _text(r.get(mapping.get("deferral_reason", ""))),
            "sop_path": _text(r.get(mapping.get("sop_path", ""))), "report_path": _text(r.get(mapping.get("report_path", ""))),
        })
    return rows, errors
...
def dataframe_to_pm_specs(df: pd.DataFrame, mapping: dict[str, str], default_pm_id: str = ""):
    ...
    for idx, r in df.iterrows():
        ...
        step = int(_num(r.get(mapping.get("step_no", ""))) or len(rows)+1); parsed = parse_spec(r.get(mapping.get("spec", "")))
        row = {
            "pm_id": pm, "step_no": step, "activity": activity, "method": _text(r.get(mapping.get("method", ""))), "unit": _text(r.get(mapping.get("unit", ""))),
            "reaction_plan": _text(r.get(mapping.get("reaction_plan", ""))), "sop_path": _text(r.get(mapping.get("sop_path", ""))),
            "sop_page": _text(r.get(mapping.get("sop_page", ""))), "sop_section": _text(r.get(mapping.get("sop_section", ""))),
        }
...
def evaluate_measurement(spec: Any, value_text: str, value_numeric: float | None) -> str:
    if getattr(spec, "input_type", "Text") == "Numeric":
        ...
        return "PASS"
    if getattr(spec, "input_type", "") == "Pass / Fail": return "PASS" if _norm(value_text) in {"pass", "ok", "yes", "good", "acceptable"} else "FAIL"
...
def calculate_next_due(schedule_type: str, frequency_value: float | None, frequency_unit: str, anchor_mode: str, original_due: datetime | None, last_completion: datetime | None, now: datetime | None = None) -> datetime | None:
    ...
    n = frequency_value or 0; units = frequency_unit.lower()
...
def parse_required_parts(text: str) -> list[tuple[str, float]]:
    """Accepts PART:2; FILTER-A x 4; SENSOR=1; bare PART defaults to 1."""
    out = []
    for token in re.split(r"[;\n,]+", text or ""):
        token = token.strip()
        if not token: continue
        m = re.match(r"^(.+?)\s*(?::|=|\bx\b|\*)\s*(\d+(?:\.\d+)?)\s*$", token, re.I)
        if m: part, qty = m.group(1).strip(), float(m.group(2))
        else: part, qty = token, 1.0
        if part: out.append((part, qty))
    return out


def pm_parts_readiness(definition: Any, availability_getter) -> list[dict[str, Any]]:
    rows = []
    for part, required in parse_required_parts(getattr(definition, "required_parts", "")):
        available = float(availability_getter(part) or 0.0)
        rows.append({"part_number": part, "required": required, "available": available, "shortage": max(0.0, required-available), "ready": available >= required})
    return rows
