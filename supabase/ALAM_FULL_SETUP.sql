-- ALAM.ph one-shot Supabase setup
-- Run this entire file in Supabase SQL Editor for the current ALAM schema.
-- Existing public `agents` table is preserved.
...
-- Comment contract: preserve stable agent-generated string IDs.
...
-- Read-only story-history access for already-published stories.
...
-- Shared ALAM content has no anonymous/client write policy. Trusted ingestion uses
-- a server-side service-role key in GitHub Actions; never expose that key to Streamlit.
