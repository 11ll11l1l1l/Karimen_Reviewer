...
def _decode(raw):
    ...
    try:
        ...
        if len(payload) > MAX_COOKIE_JSON_BYTES or not inflater.eof or inflater.unconsumed_tail or inflater.unused_data:
            return {}
...
def _save_progress(progress, manager=None):
    ...
    if manager:
        try:
            manager.set(COOKIE_NAME, _encode_for_cookie(progress), expires_at=datetime.now() + timedelta(days=365), key="set_alam_action_progress")
        except Exception:
            pass
...
def progress_counts(record):
    ...
    return (len(completed_step_keys(record) & valid), len(valid))
...
def render_action_checklist(record, manager=None):
    ...
    st.caption(f"{done}/{total} complete · Saved on this browser. These are the article’s validated action-plan steps, not new AI advice.")
    if plan.get("goal"):
    ...
    if focus and not focus["complete"]:
        ...
        effort = f" · ~{focus['remaining_minutes']} min remaining" if focus["remaining_minutes"] is not None else ""
        st.info(f"Next verified step: {next_step['title']} — {next_step['action']} · {focus['remaining']} step{'s' if focus['remaining'] != 1 else ''} left{effort}")

    for index, step in enumerate(plan["steps"], start=1):
        checked = step["key"] in completed
        value = st.checkbox(f"{index}. {step['title']}", value=checked, key=f"alam_action_{_story_key(record.get('id'))}_{step['key']}")
        meta = []
    ...
    if done == total:
        ...
        reflection_key = _reflection_key(record)
        ...
        if outcome in COMPLETION_OUTCOMES:
            ...
            for value, label in COMPLETION_OUTCOMES.items():
                if st.button(label, key=f"{reflection_key}_{value}", use_container_width=True):
                    record_completion_outcome(record, value)
                    st.rerun()
