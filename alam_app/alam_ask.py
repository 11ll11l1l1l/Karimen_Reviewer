...
Ask ALAM is retrieval-first rather than a free-form chatbot. It never answers a factual
question from model memory: displayed answer text and source links come from validated
ALAM records already loaded by the application.
"""
...
ALIASES = {
    "visa": ("immigration", "residence", "在留", "renewal"), "renew": ("renewal", "residence", "在留"),
    "pr": ("permanent residence", "永住"), "tax": ("tax", "year-end", "deduction", "税", "扶養"),
    "dependent": ("dependent", "扶養", "spouse", "relative"), "earthquake": ("earthquake", "seismic", "plate", "地震"),
    "quake": ("earthquake", "seismic", "plate", "地震"), "nisa": ("nisa", "investment", "fund"),
    "yen": ("yen", "jpy", "円", "usd/jpy"), "market": ("market", "nikkei", "topix", "jgb", "usd/jpy"),
    "scam": ("scam", "fraud", "詐欺"), "benefit": ("benefit", "allowance", "support", "給付", "手当"),
}
...
def _base_terms(query: str) -> list[str]:
    return [t.lower() for t in TOKEN_RE.findall(str(query or "")) if len(t) > 1 and t.lower() not in STOPWORDS]
...
def relevance_score(record: dict, query: str) -> float:
    ...
    if len(phrase) >= 4:
        if phrase in fields["title"]: score += 10.0
        elif phrase in fields["summary"]: score += 5.0
        elif phrase in fields["body"]: score += 2.0
    for term in terms:
        hit = False
        if term in fields["title"]: score += 5.0; hit = True
        if term in fields["tags"]: score += 3.5; hit = True
        if term in fields["summary"]: score += 2.5; hit = True
        if term in fields["body"]: score += 1.0; hit = True
...
def grounded_sources(record: dict, limit: int = 3) -> list[dict[str, str]]:
    """Return safe, deduplicated HTTP(S) citations from the validated winning record."""
    output, seen = [], set()
    for source in record.get("sources") or []:
        ...
        seen.add(url)
        publisher = str(source.get("publisher") or parsed.netloc).strip()
        title = str(source.get("title") or publisher or parsed.netloc).strip()
...
def render_ask_alam(records, comments, manager, views) -> None:
    ...
    category_by_label = {value:key for key,value in CATEGORY_LABELS.items()}; allowed = {category_by_label[label] for label in lenses if label in category_by_label}
    pool = [r for r in (records or []) if not allowed or str(r.get("_category") or r.get("category") or "") in allowed]
    ...
    st.caption("Ask ALAM performs deterministic evidence retrieval, shows the source links behind its top answer, and refuses unsupported answers. A generative synthesis layer can be added later only if it preserves these citations and the same refusal boundary.")
