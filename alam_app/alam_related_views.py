...
def related_story_candidates(record: dict[str, Any], records: list[dict[str, Any]], limit: int = 3):
    """Return deterministic ALAM-backed connections without implying causality."""
    return intelligence.connected_stories(record, records, limit=limit)
...
def render_related_stories(record: dict[str, Any], records: list[dict[str, Any]]) -> None:
    ...
    st.markdown(
        ...
        "Shared signals are context, not proof that one event caused another.</div></div>",
